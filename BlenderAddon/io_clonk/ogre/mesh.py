...
def dot_mesh(ob, path, force_name=None, **kwargs):
    ...
    with open(target_file, 'w') as f:
        ...
        for tessface in mesh.tessfaces:
            ...
            # get first triangle
            tris = [(tessface.vertices[0], tessface.vertices[1], tessface.vertices[2])]
            # and optionally second triangle
            if len(tessface.vertices) >= 4:
                tris.append((tessface.vertices[0], tessface.vertices[2], tessface.vertices[3]))
            ...
            if dotextures:
                ...
                for layer in uvcache:
                    uv1, uv2, uv3, uv4 = layer[tessface.index]
            ...
            for tidx, tri in enumerate(tris):
                ...
                for vidx, idx in enumerate(tri):
                    ...
                    # get the normal
                    if tessface.use_smooth:
                        nx, ny, nz = swap(vertex.normal)  # fixed june 17th 2011
                    else:
                        nx, ny, nz = swap(tessface.normal)

                    # get the color of the vertex
                    export_vertex_color, color_tuple = extract_vertex_color(vcolors, vcolors_alpha, tessface, idx)
                    ...
                    x, y, z = swap(vertex.co)  # xz-y is correct!
                ...
                faces = material_faces[tessface.material_index]
